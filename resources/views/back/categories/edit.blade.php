...
@section('content')
    <main class="row bg-white my-3 py-3">
        <div class="col-6 mx-auto">
            ...
            <div class="row">
                <div class="col-12">
                    <form action="{{ route('back.categories.update', $category->slug) }}" method="post">
                        @method('PATCH ')
                        @csrf
                        <div class="mb-3">
                            <label for="name" class="form-label">Name</label>
                            <input type="text" name="name" id="name" class="form-control with-slug" data-url="{{ route('back.slug') }}" value="{{ old('name', $category->name) }}" required>
                        </div>
                        <div class="mb-3">
